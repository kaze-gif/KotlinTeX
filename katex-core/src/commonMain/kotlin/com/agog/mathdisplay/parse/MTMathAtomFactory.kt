...
open class MTMathAtomFactory {
    ...
    private val delimiterValueToName = hashMapOf<String, String>()

    init {
        for ((command, delimiter) in delimiters) {
            val existingCommand: String? = delimiterValueToName[delimiter]
            ...
            delimiterValueToName[delimiter] = command
        }
    }
    ...
    private val supportedAccentedCharacters: Map<Char, Pair<String, String>> = mapOf(
        // Acute accents
        'á' to Pair("acute", "a"),
        'é' to Pair("acute", "e"),
        'í' to Pair("acute", "i"),
        ...
        'œ' to Pair("oe", ""),
        'ß' to Pair("ss", ""),
        '\'' to Pair("upquote", ""),  // this may be dangerous in math mode
    ...
    val fontStyles: HashMap<String, MTFontStyle> = hashMapOf(
    ...
    fun fontStyleWithName(fontName: String): MTFontStyle? {
    ...
    fun atomFromAccentedCharacter(ch: Char): MTMathAtom? {
        val symbol = supportedAccentedCharacters[ch]
        if (symbol != null) {
            // first handle any special characters
            val atom = atomForLatexSymbol(symbol.first)
            if (atom != null) {
                return atom
            }

            val accent = accentWithName(symbol.first)
            if (accent != null) {
                // The command is an accent
                val list = MTMathList()
                val char = symbol.second.first()
                list.addAtom(atomForCharacter(char))
    ...
    /**
     * 根据给定的类型和值创建一个 `MTMathAtom` 实例。如果类型属于特定类型，将创建特定的子类对象。
     *
     * @param type 表示数学原子类型的枚举值，用于决定创建哪种类型的对象。
     * @param value 表示该数学原子的值，通常为字符串形式。
     * @return 创建的 `MTMathAtom` 或其特定子类对象。
     */
    ...
    fun atomForCharacter(char: Char): MTMathAtom? {
        val chStr = char.toString()
        when (char) {
            // Cyrillic alphabet
            in '\u0410'..'\u044F' -> return atomWithType(
                type = MTMathAtomType.KMTMathAtomOrdinary,
                value = chStr
            )

            in supportedAccentedCharacters.keys -> // support for áéíóúýàèìòùâêîôûäëïöüÿãñõçøåæœß'ÁÉÍÓÚÝÀÈÌÒÙÂÊÎÔÛÄËÏÖÜÃÑÕÇØÅÆŒ
                return atomFromAccentedCharacter(char)

            else -> when {
                char.code < 0x0021 || char.code > 0x007E ->
                    return null
                ...
                char in listOf('(', '[') ->
                    return atomWithType(type = MTMathAtomType.KMTMathAtomOpen, value = chStr)

                char in listOf(')', ']', '!', '?') ->
                    return atomWithType(type = MTMathAtomType.KMTMathAtomClose, value = chStr)

                char in listOf(',', ';') ->
                    return atomWithType(type = MTMathAtomType.KMTMathAtomPunctuation, value = chStr)

                char in listOf('=', '>', '<') ->
                    return atomWithType(type = MTMathAtomType.KMTMathAtomRelation, value = chStr)

                char == ':' ->
                    // Math colon is ratio. Regular colon is \colon
                    return atomWithType(type = MTMathAtomType.KMTMathAtomRelation, value = "\u2236")

                char == '-' ->
                    return atomWithType(
                        type = MTMathAtomType.KMTMathAtomBinaryOperator,
                        value = "\u2212"
                    )
                ...
                char in 'a'..'z' || char in 'A'..'Z' ->
                    return atomWithType(type = MTMathAtomType.KMTMathAtomVariable, value = chStr)

                char in listOf('\'', '/', '@', '`', '|') ->
                    return atomWithType(type = MTMathAtomType.KMTMathAtomOrdinary, value = chStr)

                else -> {
                    require(false) { "Unknown ASCII character '$char'. Should have been handled earlier." }
                    return null
                }
            }
        }
    ...
    fun atomForLatexSymbol(symbolName: String): MTMathAtom? {
    ...
    fun latexSymbolName(atom: MTMathAtom): String? {
        if (atom.nucleus.isEmpty()) {
            return null
        }
    ...
    fun boundary(delimiterName: String): MTMathAtom? {
        val delimiterValue = delimiters[delimiterName] ?: return null
        return MTMathAtom(MTMathAtomType.KMTMathAtomBoundary, delimiterValue)
    }

    fun getDelimiterName(boundary: MTMathAtom): String? {
    ...
    private fun fraction(num: MTMathList, denom: MTMathList): MTFraction {
        val frac = MTFraction()
        frac.numerator = num
    ...
    fun mathListForCharacters(chars: String): MTMathList {
        val list = MTMathList()
        for (char in chars) {
            val atom = MTMathAtom.atomForCharacter(char)
            if (atom != null) {
                list.addAtom(atom)
            }
        }
    ...
    fun fraction(numStr: String, denominatorStr: String): MTFraction {
        val num = mathListForCharacters(numStr)
        val denom = mathListForCharacters(denominatorStr)
        return fraction(num, denom)
    }

    val matrixEnvs: Map<String, Array<String>> = hashMapOf(
        "matrix" to arrayOf(""),
        "pmatrix" to arrayOf("(", ")"),
        "bmatrix" to arrayOf("[", "]"),
        "Bmatrix" to arrayOf("{", "}"),
    ...
    fun table(
        ...
        if (env == null) {
            ...
            return table
        } else if (matrixEnvs.containsKey(env)) {
            ...
            // All the lists are in text style
            ...
            // Add delimiters
            val delimiters = matrixEnvs[env]
            return if (delimiters?.size == 2) {
                val inner = MTInner()
                inner.leftBoundary = boundary(delimiters[0])
                inner.rightBoundary = boundary(delimiters[1])
            ...
            val space: MTMathAtom? = atomForLatexSymbol(",")
            if (space != null) {
